...
import { sortColorsByHue, sortColorsBySaturation, sortColorsByLightness } from './utils/colorUtils';
import { getMockColors } from './data/mockColors';
...
const ColorSorterApp = () => {
  ...
  // Load initial - start clean without mock colors
  useEffect(() => {
    const loadInitialColors = async () => {
      try {
        // Empezar completamente limpio sin colores de muestra
        setColors([]);
      } catch (error) {
    ...
    loadInitialColors();
  }, []); // Remove sortOrder dependency to prevent reset // Add dependency to re-run when sortOrder changes
